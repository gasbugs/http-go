...
## 배포 및 릴리스
- `build.sh` 스크립트를 사용하여 멀티 아키텍처(arm64, amd64) 이미지를 손쉽게 빌드하고 레지스트리에 푸시할 수 있습니다. 시스템에 설치된 도구(`podman` 또는 `docker`)를 자동 감지하여 실행됩니다.
  ```bash
  # 사용법: ./build.sh <태그>
  ./build.sh v2.2
  ```
  *(주의: 빌드한 이미지를 푸시하려면 사전에 해당 레지스트리에 로그인되어 있어야 합니다.)*
